...
import com.truelife.chat.adapters.MyStatusAdapter;
...
import com.truelife.chat.utils.ImageEditorRequest;
...
public class MyStatusActivity extends BaseActivity implements ActionMode.Callback {
    ...
    public void uplaodTextStatus(TextStatus textStatus) {
        if (!NetworkHelper.isConnected(TLApplication.Companion.context())) {
            Toast.makeText(TLApplication.Companion.context(), R.string.no_internet_connection, Toast.LENGTH_SHORT).show();
        } else {

            Toast.makeText(TLApplication.Companion.context(), R.string.uploading_status, Toast.LENGTH_SHORT).show();
