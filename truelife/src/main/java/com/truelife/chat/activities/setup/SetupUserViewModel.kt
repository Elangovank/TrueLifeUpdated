...
import com.truelife.chat.extensions.getFileRx
import com.truelife.chat.extensions.observeSingleValueEvent
import com.truelife.chat.extensions.updateChildrenRx
...
class SetupUserViewModel : ViewModel() {
    ...
    private fun getUserInfoHashmap(userName: String, thumbImg: String, photoUrl: String?, filePath: String? = null): HashMap<String, Any> {
        ...
        val defaultStatus = String.format(TLApplication.context()?.getString(R.string.default_status)?:"", TLApplication.context()?.getString(R.string.app_name))
        map["status"] = defaultStatus
        val appVersion = AppVerUtil.getAppVersion(TLApplication.context())
        if (appVersion != "")
            map["ver"] = appVersion
    ...
    private fun saveCountryCode() {
        val phoneUtil = PhoneNumberUtil.createInstance(TLApplication.context())
