package com.truelife.chat.activities.main.status
...
import com.truelife.chat.activities.MyStatusActivity
import com.truelife.chat.activities.ViewStatusActivity
import com.truelife.chat.activities.main.MainActivity.Companion.CAMERA_REQUEST
import com.truelife.chat.activities.main.MainActivity.Companion.REQUEST_CODE_TEXT_STATUS
import com.truelife.chat.activities.main.MainViewModel
import com.truelife.chat.activities.main.status.StatusFragmentEvent.OnActivityResultEvent
import com.truelife.chat.activities.main.status.StatusFragmentEvent.StatusInsertedEvent
import com.truelife.chat.adapters.StatusAdapter
...
import com.truelife.chat.model.realms.UserStatuses
import com.truelife.chat.utils.*
import com.truelife.chat.utils.network.FireManager
import com.truelife.chat.utils.network.StatusManager
import com.truelife.chat.views.HeaderViewDecoration
import com.truelife.chat.views.TextViewWithShapeBackground
...
class StatusFragment : BaseFragment(), StatusAdapter.OnClickListener {
            ...
            if (resultCode == ResultCodes.IMAGE_CAPTURE_SUCCESS) {
                ...
                for (mPath in mPaths) {
                    if (!FileUtils.isFileExists(mPath)) {
                        Toast.makeText(activity, TLApplication.context()?.resources?.getString(R.string.image_video_not_found), Toast.LENGTH_SHORT).show()
                        return
                    }
                }
                ...
                if (FileUtils.isPickedVideo(mPaths[0])) {
                    ...
                    if (seconds <= MAX_STATUS_VIDEO_TIME) {
                        ...
                        Toast.makeText(activity, TLApplication.context()?.resources?.getString(R.string.video_length_is_too_long), Toast.LENGTH_SHORT).show()
                    }
                } else {
    ...
    private fun setupHeaders() {
        ...
        if (header1pos == -1) {
            header1Title = TLApplication.context()?.resources?.getString(R.string.viewed_statuses)
            header2Title = TLApplication.context()?.resources?.getString(R.string.viewed_statuses)?:""
        } else {
            header1Title = TLApplication.context()?.resources?.getString(R.string.recent_updates)
            header2Title = TLApplication.context()?.resources?.getString(R.string.viewed_statuses)?:""
        }
    }
    ...
    private fun uploadVideoStatus(path: String) {
        if (!NetworkHelper.isConnected(TLApplication.context())) {
            Toast.makeText(activity, TLApplication.context()?.resources?.getString(R.string.no_internet_connection), Toast.LENGTH_SHORT).show()
            return
        }
        ...
        disposables.add(statusManager.uploadStatus(path, StatusType.VIDEO, true).subscribe { status, throwable ->
            if (throwable != null) {
                Toast.makeText(activity, TLApplication.context()?.resources?.getString(R.string.error_uploading_status), Toast.LENGTH_SHORT).show()
            } else {
                setMyStatus()
                Toast.makeText(activity, TLApplication.context()?.resources?.getString(R.string.status_uploaded), Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun uploadImageStatus(path: String) {
        if (!NetworkHelper.isConnected(TLApplication.context())) {
            Toast.makeText(TLApplication.context(), TLApplication.context()?.resources?.getString(R.string.no_internet_connection), Toast.LENGTH_SHORT).show()
            return
        }
        Toast.makeText(TLApplication.context(), TLApplication.context()?.resources?.getString(R.string.uploading_status), Toast.LENGTH_SHORT).show()
    ...
    fun onTextStatusResult(textStatus: TextStatus) {
        if (!NetworkHelper.isConnected(TLApplication.context())) {
            Toast.makeText(TLApplication.context(), R.string.no_internet_connection, Toast.LENGTH_SHORT).show()
        } else {
            Toast.makeText(TLApplication.context(), R.string.uploading_status, Toast.LENGTH_SHORT).show()
