...
import com.truelife.chat.activities.calling.CallingActivity;
...
import  com.truelife.chat.receivers.HandleReplyReceiver;
import  com.truelife.chat.receivers.MarkAsReadReceiver;
...
import static com.truelife.chat.utils.IntentUtils.NOTIFICATION_ACTION_DECLINE;
import static com.truelife.chat.utils.MessageTypeHelper.getMessageContent;
...
public class NotificationHelper extends ContextWrapper {
    ...
    private Bitmap getProfilePhotoAsBitmap(String thumbImg) {
        if (thumbImg != null)
            return com.truelife.chat.utils.BitmapUtils.encodeImage(thumbImg);
    ...
    public void dismissNotification(String chatId, boolean decrementCount) {
        ...
        Chat chat = com.truelife.chat.utils.RealmHelper.getInstance().getChat(chatId);
        if (chat != null) {
            ...
            if (decrementCount) {
                ...
                com.truelife.chat.utils.RealmHelper.getInstance().deleteUnReadMessages(chatId);
            }
            //dismiss grouped notification if there are no notifications left
            if (!isBelowApi24() && !com.truelife.chat.utils.RealmHelper.getInstance().areThereUnreadChats()) {
    ...
    public void fireNotification(String newMessageChatId) {
        ...
        long unreadMessagesCount = com.truelife.chat.utils.RealmHelper.getInstance().getUnreadMessagesCount();
        ...
        if (isBelowApi24()) {
            ...
            List<Message> last7UnreadMessages = com.truelife.chat.utils.RealmHelper.getInstance().getLast7UnreadMessages();
            final NotificationCompat.MessagingStyle messagingStyle = new NotificationCompat.MessagingStyle("");
            //get all unread chats count
            int chatsCount = (int) com.truelife.chat.utils.RealmHelper.getInstance().getUnreadChatsCount();
            ...
            if (last7UnreadMessages.isEmpty()) {
                ...
                for (int i = 0; i < last7UnreadMessages.size(); i++) {
                    ...
                    if (chatHashMap.containsKey(chatId)) {
                        ...
                        chat = com.truelife.chat.utils.RealmHelper.getInstance().getChat(chatId);
                    ...
                    if (isNotificationsEnabled && !chat.isMuted()) {
                        ...
                        if (chat.getUser().isGroupBool()) {
                            ...
                            User user = com.truelife.chat.utils.ListUtil.getUserById(unreadMessage.getFromId(), users);
                            if (user != null) {
            ...
            List<Chat> unreadChats = com.truelife.chat.utils.RealmHelper.getInstance().getUnreadChats();
            ...
            for (Chat unreadChat : unreadChats) {
                ...
                if (isNotificationsEnabled && !unreadChat.isMuted()) {
                    ...
                    List<Message> messageList = com.truelife.chat.utils.RealmHelper.getInstance().filterUnreadMessages(unreadChat.getUnreadMessages());
                    if (isGroup) {
    ...
    private PendingIntent getPendingIntent(Chat chat) {
        ...
        if (isBelowApi24()) {
            ...
            if (chat != null) {
                ...
                intent.putExtra(com.truelife.chat.utils.IntentUtils.UID, chat.getChatId());
...
//        start chatActivity with the clicked user
    ...
    private Intent getMessageReplyIntent(String label, String chatId) {
        ...
        intent
                ...
                .setAction(com.truelife.chat.utils.IntentUtils.INTENT_ACTION_HANDLE_REPLY)
                .putExtra(KEY_PRESSED_ACTION, label)
                .putExtra(com.truelife.chat.utils.IntentUtils.UID, chatId)
                .putExtra(com.truelife.chat.utils.IntentUtils.EXTRA_CHAT_ID, chatId);
        return intent;
    }
    ...
    private Intent getMarkAsReadIntent(String chatId, boolean isGroup) {
        ...
        intent
                .setAction(com.truelife.chat.utils.IntentUtils.INTENT_ACTION_MARK_AS_READ)
                .putExtra(com.truelife.chat.utils.IntentUtils.EXTRA_CHAT_ID, chatId)
                .putExtra(com.truelife.chat.utils.IntentUtils.IS_GROUP, isGroup);
        return intent;
    }
    ...
    public void messageDeleted(Message message) {
        if (message != null) {
            ...
            if (!TLApplication.Companion.getCurrentChatId().equals(chatId)) {
    ...
    public Notification createActiveCallNotification(FireCall fireCall, int notificationId) {
        ...
        PendingIntent notificationPIntent = getNotificationClickPendingIntent(fireCall, com.truelife.chat.utils.IntentUtils.NOTIFICATION_ACTION_CLICK, PI_REQUEST_CODE_CLICK);
        ...
        Intent hangupIntent = getCallingActivityIntent(fireCall, com.truelife.chat.utils.IntentUtils.NOTIFICATION_ACTION_HANGUP);
    ...
    private Intent getCallingActivityIntent(FireCall fireCall, int action) {
        ...
        notificationIntent.putExtra(com.truelife.chat.utils.IntentUtils.CALL_DIRECTION, fireCall.getDirection());
        notificationIntent.putExtra(com.truelife.chat.utils.IntentUtils.CALL_TYPE, fireCall.getCallType());
        notificationIntent.putExtra(com.truelife.chat.utils.IntentUtils.CALL_ID, callId);
        notificationIntent.putExtra(com.truelife.chat.utils.IntentUtils.UID, fireCall.getUser().getUid());
        notificationIntent.putExtra(com.truelife.chat.utils.IntentUtils.PHONE, fireCall.getPhoneNumber());
        notificationIntent.putExtra(com.truelife.chat.utils.IntentUtils.CALL_ACTION_TYPE, action);
...
//        notificationIntent.addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
    ...
    public Notification createIncomingCallNotification(FireCall fireCall, int notificationId) {
        PendingIntent notificationPIntent = getNotificationClickPendingIntent(fireCall, com.truelife.chat.utils.IntentUtils.NOTIFICATION_ACTION_START_INCOMING, PI_REQUEST_CODE_CLICK);
