...
import com.truelife.chat.model.realms.RealmLocation;
...
public class MessageCreator {
    ...
    private static Message createTextMessage(User user, String text) {
        ...
        //messageId
        final String pushKey = com.truelife.chat.utils.FireConstants.messages.push().getKey();
    ...
    public static Message createStickerMessage(User user, final String imagePath) {
        ...
        String fileSize = com.truelife.chat.utils.Util.getFileSizeFromLong(file.length(), true);
    ...
    private static Message createImageMessage(User user, final String imagePath, boolean fromCamera) {
        ...
        File file = com.truelife.chat.utils.DirManager.generateFile(type);
        String fileExtension = com.truelife.chat.utils.Util.getFileExtensionFromPath(imagePath);

        if (fileExtension.equals("gif")) {
            try {
                com.truelife.chat.utils.FileUtils.copyFile(imagePath, file);
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
            //compress image and copy it to the given file
            com.truelife.chat.utils.BitmapUtils.compressImage(imagePath, file);
        }
        ...
        if (fromCamera) {
            ...
            com.truelife.chat.utils.FileUtils.deleteFile(imagePath);
        }
        ...
        //blurred thumb image
        String thumb = com.truelife.chat.utils.BitmapUtils.decodeImage(filePath, true);
        message.setType(type);
        message.setFromId(FireManager.getUid());
        message.setToId(receiverUid);
        message.setTimestamp(String.valueOf(new Date().getTime()));
    ...
    private static Message createVideoMessage(Context context, User user, final String path) {
        ...
        //get video size
        String videoSize = com.truelife.chat.utils.Util.getFileSizeFromLong(file.length(), true);
        String receiverUid = user.getUid();


        //get raw image thumb bitmap
        Bitmap videoThumbBitmap = com.truelife.chat.utils.BitmapUtils.getThumbnailFromVideo(path);
        //generate blurred thumb to send it to other user
        String thumb = com.truelife.chat.utils.BitmapUtils.decodeImage(videoThumbBitmap);
        ...
        //set video duration
        message.setMediaDuration(com.truelife.chat.utils.Util.getVideoLength(context, path));
    ...
    private static Message createAudioMessage(User user, String filePath, String audioDuration) {
        ...
        String fileSize = com.truelife.chat.utils.Util.getFileSizeFromLong(audioFile.length(), true);
        //get file extension
        String fileExtension = com.truelife.chat.utils.Util.getFileExtensionFromPath(filePath);
        File file = com.truelife.chat.utils.DirManager.generateAudioFile(type, fileExtension);

        try {
            //copy original file to this new path
            com.truelife.chat.utils.FileUtils.copyFile(audioFile, file);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    private static Message createFileMessage(User user, final String filePath) {
        ...
        final Message message = new Message();
        final String pushKey = com.truelife.chat.utils.FireConstants.messages.push().getKey();
        final String fileName = com.truelife.chat.utils.Util.getFileNameFromPath(filePath);
    ...
    public static Message createForwardedMessage(Message mMessage, User user, String fromId) {
        ...
        com.truelife.chat.utils.RealmHelper.getInstance().saveObjectToRealm(message);
        com.truelife.chat.utils.RealmHelper.getInstance().saveChatIfNotExists(message, user);
        return message;
    }
        ...
        public Message build() {
            ...
            switch (type) {
                ...
                case MessageType.SENT_STICKER:
                    message = createStickerMessage(user, path);
                    break;
            ...
            if (user.isBroadcastBool()) {
                ...
                for (User user : user.getBroadcast().getUsers()) {
                    ...
                    com.truelife.chat.utils.RealmHelper.getInstance().saveObjectToRealm(clonedMessage);
                    com.truelife.chat.utils.RealmHelper.getInstance().saveChatIfNotExists(clonedMessage, user);
                }
            }
