...
import static com.truelife.chat.model.constants.MessageStat.SENT;
...
public class DownloadManager {
    ...
    public static void download(final Message message, final OnComplete onComplete) {
        ...
        if (type == MessageType.RECEIVED_FILE)
            file = com.truelife.chat.utils.DirManager.generateFileForFilesType(type, com.truelife.chat.utils.Util.getFileNameFromPath(link));
        else if (type == MessageType.RECEIVED_AUDIO)
            file = com.truelife.chat.utils.DirManager.generateAudioFile(type, com.truelife.chat.utils.Util.getFileExtensionFromPath(link));
        else
            file = DirManager.generateFile(type);


        //get firebase storage ref
        StorageReference ref = com.truelife.chat.utils.FireConstants.storageRef.child(link);
            ...
            public void onComplete(@NonNull Task<FileDownloadTask.TaskSnapshot> task) {
                ...
                if (task.isSuccessful() && message.completeAfterDownload()) {
                    ...
                    if (MessageType.isVideo(type)) {
                        ...
                        if (videoThumb != null)
                            com.truelife.chat.utils.RealmHelper.getInstance().setVideoThumb(messageId, message.getChatId(), videoThumb);
                    }
                    //update downloadupload state to success
                    com.truelife.chat.utils.RealmHelper.getInstance().updateDownloadUploadStat(messageId, DownloadUploadStat.SUCCESS, file.getPath());
                    ...
                    if (exception != null && exception.getErrorCode() != StorageException.ERROR_CANCELED) {
                        com.truelife.chat.utils.RealmHelper.getInstance().changeDownloadOrUploadStat(messageId, DownloadUploadStat.FAILED);
                        updateJobCallback(false, onComplete);
                    } else {
                        updateJobCallback(true, onComplete);
                    }
                    //delete uncompleted file from device
                    com.truelife.chat.utils.FileUtils.deleteFile(file.getPath());
    ...
    public static void cancelDownload(Message message) {
        ...
        if (downloadTaskHashmap.containsKey(messageId)) {
            ...
            downloadTaskHashmap.remove(messageId);
            com.truelife.chat.utils.FileUtils.deleteFile(message.getLocalPath());
        }

        removeProgressFromHashmap(messageId);
        com.truelife.chat.utils.RealmHelper.getInstance().changeDownloadOrUploadStat(messageId, DownloadUploadStat.CANCELLED);
    ...
    public static void cancelDownload(String messageId) {
        Message message = com.truelife.chat.utils.RealmHelper.getInstance().getMessage(messageId);
        if (downloadTaskHashmap.containsKey(messageId)) {
            FileDownloadTask fileDownloadTask = downloadTaskHashmap.get(messageId);
            fileDownloadTask.cancel();
            downloadTaskHashmap.remove(messageId);
    ...
    public static void sendMessage(final Message message, final OnComplete onComplete) {
        ...
        com.truelife.chat.utils.FireConstants.getMessageRef(message.isGroup(), message.isBroadcast(), message.getChatId()).updateChildren(childUpdates).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                ...
                if (task.isSuccessful()) {
                    ...
                    if (message.isBroadcast()) {
                        RealmResults<Message> broadcastedMessages = com.truelife.chat.utils.RealmHelper.getInstance().getMessages(message.getMessageId());
                        for (Message broadcastedMessage : broadcastedMessages) {
                            com.truelife.chat.utils.RealmHelper.getInstance().updateMessageStatLocally(broadcastedMessage.getMessageId(), broadcastedMessage.getChatId(), SENT);
                        }
                    } else {
                        com.truelife.chat.utils.RealmHelper.getInstance().updateMessageStatLocally(pushKey, SENT);
                    }
                ...
                if (task.isSuccessful() && message.completeAfterDownload()) {
                    ...
                    FireConstants.getMessageRef(message.isGroup(), message.isBroadcast(), message.getChatId()).updateChildren(childUpdates).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> mTask) {
                            ...
                            com.truelife.chat.utils.RealmHelper.getInstance().updateDownloadUploadStat(pushKey, mTask.isSuccessful() ? DownloadUploadStat.SUCCESS : DownloadUploadStat.FAILED);
                    ...
                    StorageException exception = (StorageException) task.getException();
                    if (exception != null && exception.getErrorCode() != StorageException.ERROR_CANCELED) {
                        com.truelife.chat.utils.RealmHelper.getInstance().changeDownloadOrUploadStat(pushKey, DownloadUploadStat.FAILED);
                        updateJobCallback(false, onComplete);
                    } else {
                        updateJobCallback(true, onComplete);
    ...
    private static void setMessageContent(String filePath, Message message) {
        try {
            ...
            com.truelife.chat.utils.RealmHelper.getInstance().changeMessageContent(message.getMessageId(), filePath);
