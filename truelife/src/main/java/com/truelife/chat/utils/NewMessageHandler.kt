...
import com.truelife.chat.model.constants.*
...
class NewMessageHandler(val context: Context, val fireManager: FireManager, val disposables: CompositeDisposable) {
    ...
    fun handleNewMessage(phone: String, message: Message) {
        ...
        if (chatId != TLApplication.getCurrentChatId()) {
    ...
    private fun saveMessageAndUpdateCount(message: Message, phone: String) {
        ...
        if (NotificationHelper.isBelowApi24() && TLApplication.getCurrentChatId() == message.chatId) message.isSeen = true
        ...
        if (TLApplication.getCurrentChatId() != message.chatId) RealmHelper.getInstance().saveUnreadMessage(message.messageId, message.chatId)
    }
    ...
    fun handleNewCall(fireCall: FireCall) {
        ...
        if (TLApplication.isIsCallActive() || TimeHelper.isTimePassedBySeconds(System.currentTimeMillis(), fireCall.timestamp, CallsManager.CALL_TIEMOUT_SECONDS)) {
