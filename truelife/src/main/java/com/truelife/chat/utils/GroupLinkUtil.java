...
public class GroupLinkUtil {
    ...
    private static String generateNewKey(String groupId) {
        return com.truelife.chat.utils.FireConstants.groupsLinks.child(groupId).push().getKey();
    }

    public static String getFinalLink(String newKey) {
        String host = "http://" + TLApplication.Companion.context().getString(R.string.group_invite_host);
        return host + "/" + newKey;
    }
        ...
        getCurrentLink(groupId, new FetchCurrentLinkCallback() {
            ...
            public void onFetch(String groupLink) {
                ...
                if (groupLink == null) {
                    ...
                    //delete old link
                    com.truelife.chat.utils.FireConstants.groupsLinks.child(groupLink).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
    ...
    private static void saveToDatabase(final String groupId, final String newKey, final GenerateLinkCallback callback) {
        com.truelife.chat.utils.FireConstants.groupLinkById.child(groupId).setValue(newKey)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        com.truelife.chat.utils.FireConstants.groupsLinks.child(newKey).setValue(groupId).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
    ...
    private static void getCurrentLink(final String groupId, final FetchCurrentLinkCallback callback) {
        com.truelife.chat.utils.FireConstants.groupLinkById.child(groupId).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
