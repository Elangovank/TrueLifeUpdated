...
import com.truelife.chat.activities.calling.AgoraConfig
...
import com.truelife.chat.activities.calling.model.*
import com.truelife.chat.common.extensions.abandonAudioFocusCompat
import com.truelife.chat.common.extensions.requestAudioFocusCompat
...
import com.truelife.chat.utils.network.CallsManager
...
class CallingService : Service(), ProximitySensor.Delegate, AudioManager.OnAudioFocusChangeListener, DuringCallEventHandler {
    ...
    private var isCameraFlip = false
                ...
                when (action) {
                    IntentUtils.NOTIFICATION_ACTION_START_INCOMING -> {
                        ...
                        RealmHelper.getInstance().getFireCall(callId)?.let { fireCall ->
                            ...
                            if (TLApplication.isIsCallActive()) return@let

                            TLApplication.setCallActive(true)
    ...
    private fun stop() {
        ...
        TLApplication.removeEventHandler(this@CallingService)
    ...
    private fun setCallActive(mCallActive: Boolean) {
        ...
        TLApplication.setCallActive(mCallActive)
    }
    ...
    fun startCall(fireCall: FireCall, isIncoming: Boolean) {
        ...
        TLApplication.addEventHandler(this@CallingService)
        ...
        fun setStateEvent(stateEvent: CallingStateEvent) {
            ...
            when (stateEvent) {
                ...
                is CallingStateEvent.FlipCameraClicked -> {
                    ...
                    postViewEvent(CallingViewState.FlipCamera(isCameraFlip))
                }
                ...
                is CallingStateEvent.OnStart -> {
                    TLApplication.phoneCallActivityResumed()
                ...
                is CallingStateEvent.OnStop -> {
                    TLApplication.phoneCallActivityPaused()
                    if (isVideoCall) {
                        videoUids.keys.forEach { uid ->
